...
from utils import parse_content, sanitize_filename
...
from uploader import upload_video, upload_photo, upload_document
...
def setup_handlers(app: Client):
    ...
    async def start_cmd(client: Client, message: Message):
        await message.reply_text(
            "🚀 **Advanced M3U8 Downloader Bot v8.0 SUPERCHARGED**\n\n"
            "✨ **Premium Features:**\n"
            "⚡ 3-4x Faster Downloads\n"
            "📊 Real-time Upload Progress\n"
            "🎯 Range Selection Support\n"
            "🔢 Auto Serial Numbering\n"
            "🎥 Multi-format: M3U8, MPD, MP4, MKV\n"
            "🖼️ Images: PNG, JPG, GIF, WEBP\n"
            "📄 Documents: PDF, DOC, TXT, ZIP\n"
            "🖼️ Enhanced Thumbnails (Never Blank!)\n"
            "⏱️ Accurate Duration Tracking\n"
            "📈 Live Progress Bars\n"
            "🔄 Parallel Processing\n\n"
    ...
    async def quality_cb(client: Client, callback: CallbackQuery):
        ...
        await callback.message.edit_text(
            f"🚀 **SUPERCHARGED Batch Download Started!**\n\n"
            f"⚡ Quality: {quality}\n"
            f"📊 Range: {start}-{end}\n"
...
async def process_batch(
    ...
    """Process batch of downloads with enhanced speed"""
    success = 0
    failed = 0
    ...
    await message.reply_text(
        ...
        f"📊 Total: {len(items)}\n"
        f"📍 Range: {start}-{end}\n\n"
        f"🚀 Powered by SUPERCHARGED Engine!"
    )
...
async def process_video(
    ...
    try:
        ...
        if not vpath or not active_downloads.get(user_id, False):
            await prog.delete()
            await message.reply_text(f"❌ Download failed: {caption}\n🔗 {item['url']}")
            return False
        
        if not os.path.exists(vpath) or not validate_video_file(vpath):
            await prog.delete()
            await message.reply_text(f"❌ Invalid video: {caption}\n🔗 {item['url']}")
            return False
        ...
        await prog.edit_text("📤 Starting upload...")
...
async def process_image(
    ...
    try:
        ...
        if not os.path.exists(ipath):
            await prog.delete()
            await message.reply_text(f"❌ File not found: {caption}")
            return False
...
def cleanup_user_data(user_id: int, file_path: str):
    ...
    # Remove temp files
    for pattern in [f"temp_{user_id}_*", f"thumb_{user_id}_*"]:
